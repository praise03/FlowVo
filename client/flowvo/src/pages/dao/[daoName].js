...
export default function ViewDAO() {
    ...
    const [currentDAO, setDAO] = useState([]);

    const [proposalCollection, setProposalCollection] = useState([]);

    const router = useRouter();

    const name = router.query.daoName
    
    async function fetchDao() {

        try {
            const daoRef = await fetchDAO(name)
            setDAO(daoRef)
        } catch (error) {
            console.log(error)
        }
    ...
    async function fetchCollection() {
        ...
        try {
            const proposalArray = await fetchProposalCollection(name)
            setProposalCollection(proposalArray)
        } catch (error) {
            console.log(error)
        }
    }

    async function vote(choice, proposalId) {
        try {
          const voteOnProposalTx = await voteOnProposal(name, (Number(proposalId)+1), choice)
          await fcl.tx(voteOnProposalTx).onceSealed()
    ...
    if (currentDAO.length == 0) { fetchDao() }
    if (proposalCollection.length == 0) { fetchCollection() }
        ...
        <section className='mt-8 lg:mt-0'>
          <div id="content" class="pb-6 pt-4">
            <div class="px-0 md:px-4 mx-auto max-w-[1012px]">
                          ...
                          <div>
                            ...
                            <div class="mt-2 truncate text-left">
                              <h3 class="my-0 flex items-center text-2xl leading-[44px] lg:text-lg">
                                <div class="mr-1 mt-2 truncate">{currentDAO.name}</div>
                              </h3>
                              <div class=" flex space-x-5 mt-2 mb-5">
                                <a href={currentDAO.twitter} target="_blank">
                                    <Image
              ...
              {
            proposalCollection.length === 0 ? (
            <p>.</p>
            ) : (
                    proposalCollection.map((proposal, proposalID) => (
                        <div key={proposalID} id={proposalID} class="relative float-right w-full pl-0 lg:w-3/4 lg:pl-5">
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <div>
                                ...
                                <div class="">
                                    { proposal.options.map((option, optionid) => (
                                        <button id={optionid}  key={optionid} onClick={() => vote(optionid, proposalID)} class="hover:bg-gray-300 cursor-pointer rounded-md relative mt-1 border-b-2 w-full" type="button">
                                        <div
                                        ...
                                        <div class="absolute right-0 mr-3 leading-[40px] text-skin-link">{Number(proposal.votes[optionid]).toFixed(2)} Votes
                                        </div>
                                ...
                                {" "}
                                            {new Date(
                                              parseInt(proposal.expirationTime) * 1000
                                            ).toLocaleDateString()}
                                </span>
                                </div>
                            </div>
